...
function find_address(hash, caseSensitive, cb) {
  if (caseSensitive) {
    ...
    // try exact match first (fast), then fall back to case-insensitive if not found
    Address.findOne({a_id: hash}).then((address) => {
      if (address)
        return cb(address);
      else {
        // fall back to case-insensitive search only if exact match not found
        // escape special regex characters to prevent injection
        const escapedHash = escapeRegex(hash);
        Address.findOne({a_id: {$regex: `^${escapedHash}$`, $options: 'i'}}).then((address) => {
          if (address)
            return cb(address);
          else
            return cb();
...
// find masternode by txid and optional addr
function find_masternode(txhash, addr, cb) {
  // build query - only include addr if it's not null/undefined
  const query = { txhash: txhash };
  if (addr != null) {
    query.addr = addr;
  }

  Masternode.findOne(query).then((masternode) => {
    if (masternode)
  ...
  save_masternode: function (raw_masternode, object_key, cb) {
    ...
    if (txhash == null && addr == null) {
      try {
        ...
        // 3) status only (e.g., "ENABLED", "POSE_BAN", "NEW_START_REQUIRED", "EXPIRED") - used by Pepe/protx masternodes
        const parts = raw_masternode.trim().split(/\s+/);
        const result = {};

        // check if the raw_masternode is a simple status-only format (Pepe/protx style)
        // these are typically single-word status values like: ENABLED, POSE_BAN, NEW_START_REQUIRED, EXPIRED
        const simpleStatuses = ['ENABLED', 'POSE_BAN', 'NEW_START_REQUIRED', 'EXPIRED', 'PRE_ENABLED', 'WATCHDOG_EXPIRED', 'OUTPOINT_SPENT', 'UPDATE_REQUIRED', 'SENTINEL_PING_EXPIRED'];
        if (parts.length === 1 && simpleStatuses.includes(parts[0].toUpperCase())) {
          result['status'] = parts[0];

          // check if the object_key is set to extract txhash and outidx
          if (object_key != null) {
          ...
          // set the txhash value (addr will remain null for simple status format)
          txhash = result['txhash'];

          // update the raw_masternode object
          raw_masternode = result;
        } else {
          // parse as the traditional space-separated format
            ...
            if (index == 0) {
              ...
              if (Number.isInteger(Number(value))) {
                ...
                if (result['lastseen'] == null) {
                  ...
                  // lastseen was already found so this must be the activetime
              ...
              if (Number.isInteger(Number(value))) {
                ...
                if (result['activetime'] == null) {
                  ...
                  // activetime was already found so this must be the lastpaid time
              ...
              if (Number.isInteger(Number(value))) {
                ...
                if (result['lastpaid'] == null) {
                  ...
                  // lastpaid was already found so this must be the lastpaidblock
